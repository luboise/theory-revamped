...
async function readResource(filename) {
  try {
    ...
    filehandle = fs.openSync(`./resources/${filename}`, "r");
...
app.get("/api/song/:song_id/:diff", async function (req, res) {
  try {
    ...
    const methodObjects = JSON.parse(await readResource("method_objects.json"));

    const songData =
      req.params.song_id in songObjects ? songObjects[req.params.song_id] : {};
